...
from .geometry import (
    compute_epipolar_lines_to_plot_from_F,
    compute_fundamental_from_essential,
)
from .metrics import check_epipolar_constraint, compute_sampson_error


def plot_imgs(images, cmap=None, titles=None, figsize=None, rows=1):
    if figsize is None:
        figsize = (3 * len(images) // rows, 5 * rows)
    fig, ax = plt.subplots(rows, (len(images) + rows - 1) // rows, figsize=figsize)
    ax = ax.flatten()
...
def plot_imgs_and_kpts(
    ...
    separator = (
        torch.ones(
            (target_h, space, 3), dtype=img1_padded.dtype, device=img1_padded.device
        )
        * 255
    )
...
def plot_imgs_and_epipolar_lines(
    img1,
    img2,
    points1,
    points2,
    ...
    points2 = (
        to_torch(points2, b=False)
        if points2 is not None
        else torch.zeros_like(points1) - 1
    )
    E12 = to_torch(E12)
    ...
    if verbose:
        print(
            "Epipolar constraint (all points) x'Fx=0:",
            check_epipolar_constraint(E12, points1, points2),
        )
        print(
            "Sampson error (all points):             ",
            compute_sampson_error(E12, points1, points2),
        )
        print(
...
def plot_bar_from_df(
    ...
    ax.set_xticks(
        np.arange(len(df)) + 0.25
    )  # Set ticks at the center of each bar group
    ax.set_xticklabels(
        df.index, rotation=45, ha="right", rotation_mode="anchor"
    ...
    if models is None:
        models = (
            df.columns if df2 is None else df2.columns.tolist() + df.columns.tolist()
        )
