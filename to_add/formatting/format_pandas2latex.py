...
def add_colorcell_to_highest_value_multicolumn(df, cols, suffix=["\cellcolor{ForestGreen}", "\cellcolor{SpringGreen}"], mode="max"):
    """
    ...
    assert len(suffix) <= (len(cols)-invalid), f"suffix must be less or equal to the number of non nan-columns, suffix: {len(suffix)}, non nans-columns: {len(cols)-invalid}"
    ...
    for i in range(len(df_color_cells)):
        if mode == "max":
            ...
            idxs = df_color_cells.iloc[i].argsort().values[:len(suffix)] # invert order first
    ...
    for k in to_change.keys(): # 0,1
        for row,col in enumerate(to_change[k]): # row:[0,25], col, len(suffix)
                df_color_cells[df_color_cells.columns[col]][[row]] = suffix[k] + df_color_cells[df_color_cells.columns[col]][row]
...
def sort_columns(columns, first_part_order, second_part_order):
    ...
    sorted_columns = sorted(split_columns, key=lambda x: (second_part_order.index(x[1]) if x[1] in second_part_order else len(second_part_order), first_part_order.index(x[0])))
...
def add_colorcell_to_highest_value(df, cols, suffix, mode="max"):
    ...
    for col,row in to_change:
        df_color_cells.loc[df_color_cells.index[row], col] = suffix + df_color_cells.loc[df_color_cells.index[row], col]
